...
Your goal is to analyze a sequence of screenshots extracted from a screen recording of a mobile application.
The application UI may be in Chinese, English, or other languages. You must be able to read and understand Chinese characters on the UI to correctly identify screens and actions.

You need to reconstruct the "User Flow Sitemap" by following these steps:

1. **Identify Screens**: Identify distinct, unique "Screens". Ignore transition frames (blur, half-swiped pages, loading spinners) or duplicates.
...
3. **Language Handling**: 
   ...
   - For 'label', use the most prominent title or text found on the screen (keep it in the original language, e.g., if UI is Chinese, label is Chinese).
   - For 'description', briefly describe the screen's purpose in the same language as the UI.
   - For 'edges.label', describe the action (e.g., "点击[按钮]" or "Click [Button]").
      ...
      edges: {
        ...
        items: {
          ...
          properties: {
            ...
            label: { type: Type.STRING, description: "The user action (e.g. 'Tap Login', 'Swipe Right')" }
          },
